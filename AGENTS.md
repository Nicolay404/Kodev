...
## 2026-07-21 - Integración final del Frontend React con el Backend For Frontend (BFF)

- **Eliminación absoluta de Mock Data:** Se limpiaron por completo los componentes `Dashboard.tsx`, `PatientList.tsx` y `ViewRequestModal.tsx`, eliminando arreglos de datos quemados (ej. `mockPatients`, `mockDashboardData`) para asegurar que todo lo renderizado en pantalla provenga exclusivamente del backend real.
- **Implementación de capa de red (Axios):** Se refactorizó `apiClient.ts` para que apunte exclusivamente a `http://localhost:8000` (BFF) cumpliendo la regla arquitectónica de que el Frontend nunca toque el Gateway ni los microservicios directamente. Se añadieron interceptores de Axios para inyectar automáticamente el JWT (`samr_jwt_token`) en la cabecera `Authorization` de cada petición protegida.
- **Integración con React Query:** Se migraron los Custom Hooks (`useDashboard`, `usePatients`, `useTeleconsults`) para utilizar `useQuery` de `@tanstack/react-query`. Esto centraliza el manejo de estados de carga (`isLoading`), errores (`isError`) y caché global, haciendo peticiones HTTP a endpoints reales a través del BFF.
...
## 2026-07-22 — Endurecimiento de seguridad (`sec/security-hardening`)

- Se corrigió el hallazgo crítico de la auditoría de seguridad: ningún servicio fijaba `DJANGO_SETTINGS_MODULE`, por lo que los 12 microservicios corrían con `DEBUG=True` sin importar el entorno, exponiendo stack traces completos ante cualquier error 500 (OWASP A05:2021). Se fijó `DJANGO_SETTINGS_MODULE=config.settings.production` por servicio en `docker-compose.yml`.
- `ALLOWED_HOSTS` dejó de aceptar el wildcard `'*'` por defecto en los 12 `production.py`; ahora exige una lista explícita por servicio (`localhost` para clientes vía Nginx, `nginx` para las llamadas agregadas del BFF, y el nombre propio de cada servicio para M2M directo).
- Se detectó y corrigió un bug sistémico de migraciones en 10 servicios (`patient-service`, `solicitud-service`, `monitoring-service`, `evaluacion-service`, `teleconsult-service`, `emergency-service`, `cierre-caso-service`, `historial-interop-service`, `audit-service`, `admin-integracion-service`): sus `0001_initial` fijaban nombres de tabla estandarizados (ej. `patient_app_patient` → `patients`) sin una migración de renombrado, dejando bases ya inicializadas con el nombre viejo mientras el modelo actual esperaba el nuevo (500 `relation ... does not exist`). Se agregó una migración `0002`/`0003` por servicio con `ALTER TABLE IF EXISTS ... RENAME TO ...`, reversible y no-op en bases nuevas.
- El token M2M interno (`X-Service-Token`) nunca tuvo un valor real configurado: los 10 servicios que lo verifican (`SERVICE_TOKEN` en `config/settings/base.py`) caían en el default hardcodeado `'samr-internal-service-token-default'`, público en el código fuente. Se generó un valor real con `openssl rand -hex 32`, se agregó a `.env`/`.env.example` y se propagó por `docker-compose.yml` a los 10 servicios correspondientes.
- El registro público ahora exige `terms_accepted=true` (LOPDP Ecuador — Registro Oficial Suplemento 459, 2021-05-26): `auth-service` audita `terms_accepted_at` y `terms_version` por usuario, y el frontend (`Register.tsx`) agrega un checkbox obligatorio con un modal legal que cubre responsable del tratamiento, categoría de datos de salud (Art. 9), uso de IA en evaluación de riesgo/recomendaciones, y derechos del titular (Arts. 9–15).
- Se agregaron cabeceras CORS a `nginx/samr.conf` (el Gateway nunca las tuvo, porque solo lo llamaban el BFF/apps móviles/IoT hasta que el frontend empezó a usar `gatewayClient.ts` para hablarle directo desde el navegador): origin reflejado solo si coincide con la lista permitida, nunca wildcard con `credentials`, y manejo explícito de preflight `OPTIONS`.
- Pendiente: la persistencia en Supabase quedó en pausa por inestabilidad intermitente del pooler compartido (`aws-1-us-west-2.pooler.supabase.com`) — mismas credenciales, a veces conecta por `psql`/`psycopg2` aislado y falla en el servicio real con `ENOIDENTIFIER`. No se determinó si es un límite del plan gratuito o una falla del lado de Supabase.
