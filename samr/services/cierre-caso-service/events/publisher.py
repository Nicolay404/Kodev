"""Publicador de eventos de dominio SAMR."""
import json
import os
import uuid
...
def publicar_evento(routing_key: str, payload: dict) -> str:
    ...
    try:
        ...
        channel.basic_publish(
            exchange="samr.events", routing_key=routing_key,
            body=json.dumps(evento, default=_json_default), mandatory=True,
            properties=pika.BasicProperties(
                delivery_mode=2, content_type="application/json", content_encoding="utf-8",
                message_id=evento["event_id"], type=routing_key,
            ),
        )
