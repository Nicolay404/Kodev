...
export function TeleconsultRoom() {
  ...
  const [professionalIdInput, setProfessionalIdInput] = useState('');
  ...
  const handleStartSession = async (e: React.FormEvent) => {
    ...
    try {
      const created = await createTeleconsultSession(
        patientIdInput.trim(),
        role === 'center_admin' ? professionalIdInput.trim() : undefined
      );
      setSession(created);
    } catch {
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <form onSubmit={handleStartSession} className="w-full max-w-sm bg-surface p-6 rounded-lg border border-gray-200 shadow-sm space-y-4">
          ...
          {role === 'center_admin' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">ID del profesional a asignar</label>
              <input
                type="text"
                required
